...
CLASS lcl_unit_test IMPLEMENTATION.
  ...
  METHOD returns_database_amount.
    cl_abap_unit_assert=>assert_equals(
      ...
      act = NEW zcl_money(
        amount = '10550'
        database_value_decimals = 4
        currency = 'JPY' )->get_database_amount( ) ).
  ENDMETHOD.
...
CLASS lcl_int_test IMPLEMENTATION.
  METHOD converts_to_amount.
    ...
    cl_abap_unit_assert=>assert_equals(
      exp = '94'
      act = money->convert_to_amount( target_currency = 'USD' ) ).
  ENDMETHOD.
  ...
  METHOD converts_to_database_amount.
    ...
    cl_abap_unit_assert=>assert_equals(
      exp = '0.9400'
      act = money->convert_to_database_amount( target_currency = 'USD' ) ).
  ENDMETHOD.
  ...
  METHOD converts_to_money.
    ...
    cl_abap_unit_assert=>assert_equals(
      exp = `106.3830`
      act = converted_money->get_amount( ) ).

    cl_abap_unit_assert=>assert_equals(
      exp = 'EUR'
      act = converted_money->get_currency( ) ).
